 ...
 *
 *
 * UseSentencePieceTokenizer.encode() is a port of `EncodeAsIds` from the
 * SentencePiece library (https://github.com/google/sentencepiece). Encode uses
 * the Viterbi algorithm to find the most likely sequence of tokens that
 * comprise the input. For more details, refer to
 * https://arxiv.org/pdf/1804.10959.pdf.
 *
 * This is NOT a generic tokenizer: it builds a trie over a SentencePiece-like
 * vocabulary and performs Viterbi-style segmentation over it. It is therefore
 * only meaningful for the USE family of models (`use-*` profiles).
 *
 * =============================================
 */
...
// The first tokens are reserved for unk, control symbols, and user-defined symbols.
...
export class UseSentencePieceTokenizer {
  trie: Trie;
...
export async function loadTokenizer(pathToVocabulary: string) {
  ...
  const tokenizer = new UseSentencePieceTokenizer(vocabulary);
  return tokenizer;
}

/**
 * Load a vocabulary for the USE SentencePiece tokenizer.
 *
 * @param pathToVocabulary Path to the 8k vocabulary used by the USE models.
 */
